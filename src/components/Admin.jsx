...
export default function Admin({ state, actions, showToast }) {
 ...
 let wSum = 0, wTot = 0, rawCount = 0
 // Only count votes from evaluators assigned to this section
 ...
 rawCount++
 if (sectionEvals.includes(evalName)) { wSum += score; wTot++ }
 })
 return { ...p, rawCount, lockedCount: rawCount, avg: wTot > 0 ? (wSum / wTot).toFixed(2) : null }
 ...
 {[['Presentations', Object.keys(ALL_PRES).length, 'c-blue'], ['Evaluators', (state.evaluators||[]).length, 'c-gold'], ['Attendees', (state.attendees||[]).length, 'c-purple'], ['Confirmed votes', totalVotes, 'c-green'], ['Pending', pendingVotes, 'c-gold'], ['Sections', SECTIONS.length, 'c-cyan']].map(([lbl, val, cls]) => (
